...
- Кнопка «🖼 Сгенерировать картинку» с вводом промпта и вызовом ai_client.ai_generate_image
"""
...
awaiting_img_prompt_until: Optional[datetime] = None   # промпт генерации картинки
...
# --- Главное меню (добавлена кнопка генерации изображения) ---
def get_start_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🤖 ИИ", callback_data="ai_home")],
        [InlineKeyboardButton("✅ Предпросмотр", callback_data="approve")],
        [InlineKeyboardButton("🖼 Сгенерировать картинку", callback_data="gen_image_open")],
...
# --- Клавиатура предпросмотра (добавлена кнопка генерации изображения) ---
...
def build_tg_final(body_text: str | None, for_photo_caption: bool) -> str:
    ...
    limit = 1024 if for_photo_caption else 4096
...
# --- GitHub upload и публикации в TG/X ---
...
# --- Генерация изображения (универсальная) ---
...
# --- Обёртки текста под разные режимы ---
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    global last_button_pressed_at, last_action_time, manual_expected_until, awaiting_hashtags_until, awaiting_img_prompt_until
    data = q.data
    ...
    if data == "cancel_to_main":
        ...
        awaiting_img_prompt_until = None
    ...
    # Новый поток: открываем диалог ввода промпта для картинки
    if data == "gen_image_open":
        awaiting_img_prompt_until = now + timedelta(minutes=5)
        await safe_send_message(
            approval_bot, chat_id=_approval_chat_id(),
            text="🖼 Введите описание картинки (EN/RU/UA). У вас 5 минут."
        )
        return
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    global last_button_pressed_at, manual_expected_until, awaiting_hashtags_until, awaiting_img_prompt_until
    now = datetime.now(TZ)
    last_button_pressed_at = now
    ...
    # Если ждём PROMPT для генерации изображения
    if awaiting_img_prompt_until and now <= awaiting_img_prompt_until:
        prompt = (update.message.text or update.message.caption or "").strip()
        if not prompt:
            await safe_send_message(approval_bot, chat_id=_approval_chat_id(), text="⚠️ Введите текстовое описание картинки.")
            return
        warn_img, url = await _generate_ai_image_explicit(prompt)
        header = "Предпросмотр (изображение по вашему описанию)"
        if warn_img:
            header += f" — {warn_img}"
        await send_single_preview(post_data.get("text_en") or "", post_data.get("ai_hashtags") or [], header=header)
        awaiting_img_prompt_until = None
