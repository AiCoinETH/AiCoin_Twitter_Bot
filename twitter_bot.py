...
# ====== Новый edit_post с подтверждением и предпросмотром ======
async def edit_post_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_self_post and user_self_post[user_id]['state'] == 'wait_edit':
        ...
        if image_url:
            user_self_post[user_id]['image'] = image_url
        user_self_post[user_id]['state'] = 'edit_confirm'

        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("💾 Сохранить", callback_data="save_edit_post")],
            [InlineKeyboardButton("✏️ Ещё редактировать", callback_data="edit_post")],
            [InlineKeyboardButton("❌ Отмена", callback_data="cancel_to_main")]
        ])
        try:
            await send_photo_with_download(
                approval_bot,
                TELEGRAM_APPROVAL_CHAT_ID,
                user_self_post[user_id].get("image", post_data["image_url"]),
                caption=user_self_post[user_id].get("text", post_data["text_ru"]),
                reply_markup=keyboard
            )
        except Exception as e:
            logging.error(f"Ошибка предпросмотра после редактирования: {e}")
        return
...
async def message_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in user_self_post:
        ...
        if state == 'edit_confirm':
            # Если пользователь ещё раз что-то прислал, даём предпросмотр заново
            await edit_post_message_handler(update, context)
            return
        if state == 'wait_post':
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "save_edit_post":
        ...
        if info and info.get('state') == 'edit_confirm':
            post_data["text_ru"] = info.get("text", post_data["text_ru"])
            post_data["image_url"] = info.get("image", post_data["image_url"])
    ...
    # ----- остальной button_handler без изменений -----
