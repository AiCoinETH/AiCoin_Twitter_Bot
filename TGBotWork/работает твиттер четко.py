...
Ключевое:
- Telegram: хвост добавляется ВСЕГДА (и не дублируется).
- Twitter:
    VERBATIM_MODE=True  -> публикуем РОВНО то, что прислал пользователь (без хвостов).
    VERBATIM_MODE=False -> добавляем хвост (🌐 site | 🐺 Telegram) + дедуп хэштегов; лимит 275.

Поддерживаем 4 сценария: текст / картинка / текст+картинка / только картинка.
...
Опционально:
  AICOIN_WORKER_URL, PUBLIC_TRIGGER_SECRET, AICOIN_WORKER_SECRET, LOG_LEVEL
"""
...
# По умолчанию: как написал — так и публикуем в X.
VERBATIM_MODE = True

# -----------------------------------------------------------------------------
# Константы хвостов
# -----------------------------------------------------------------------------
# X: хвост с волком → ссылка на Telegram
TW_TAIL_REQUIRED = "🌐 https://getaicoin.com | 🐺 https://t.me/AiCoin_ETH"
# TG: HTML‑хвост (всегда добавляем, без дублей)
...
# ---------- ХВОСТ ДЛЯ X (если VERBATIM_MODE=False) ----------
...
def ensure_tg_tail(text_html: str | None) -> str:
    """Добавляет TG‑хвост ВСЕГДА, но без дублей."""
    base = _strip_html(text_html or "")
    low = base.lower()
    # если пользователь сам уже вставил оба домена — не дублируем
    if ("getaicoin.com" in low) and ("x.com/aicoin_eth" in low):
        return base
    if base:
        return f"{base}\n\n{TG_TAIL_HTML}"
    return TG_TAIL_HTML
...
def build_telegram_preview(text_en: str, _ai_hashtags_ignored=None) -> str:
    # Показываем и публикуем с хвостом всегда
    raw = (text_en or "").strip()
    safe = html_escape(raw)  # только пользовательский текст экранируем; хвост — валидный HTML
    return ensure_tg_tail(safe)
...
# -----------------------------------------------------------------------------
# Отправка в Telegram (канал) — хвост ВСЕГДА
# -----------------------------------------------------------------------------
async def publish_post_to_telegram(text: str | None, image_url: Optional[str] = None) -> bool:
    """Публикуем в канал Telegram: текст/картинка/оба/только картинка. Хвост добавляется ВСЕГДА (без дублей)."""
    try:
        final_html = ensure_tg_tail(text or "")
        if image_url:
...
# -----------------------------------------------------------------------------
# Отправка в Twitter/X — 4 кейса, корректные аргументы Tweepy
# -----------------------------------------------------------------------------
...
def publish_post_to_twitter(text_en: str | None, image_url: str | None = None, ai_hashtags=None) -> bool:
    """
    Публикация в X:
      - VERBATIM_MODE=True  → ровно то, что прислал пользователь (текст/медиа/оба/только медиа).
    ...
    try:
        ...
        # нет ни медиа, ни текста — сообщаем и выходим
        ...
        # только текст
        if not media_ids and clean_text:
        ...
        # Часто бывает 403 Forbidden (доступ у приложения/ключей)
        log.error(f"Ошибка публикации в Twitter (Tweepy): {e}")
        asyncio.create_task(send_with_start_button(
            TELEGRAM_APPROVAL_CHAT_ID,
            f"❌ Twitter (Tweepy): {e}\nПроверь ключи/доступ приложения (Read+Write+DM) и токены аккаунта."
        ))
        return False
    except Exception as e:
...
# -----------------------------------------------------------------------------
# Отправка предпросмотра в чат согласования (единая карточка)
# -----------------------------------------------------------------------------
