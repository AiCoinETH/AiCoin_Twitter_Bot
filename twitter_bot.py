...
- ✅ Картинки и видео: TG — send_photo/send_video; X — media_upload (image) и chunked upload v1.1 (video).
...
- 🆕 Диагностика окружения (Gemini/Vertex/TG/Twitter/GitHub) + кнопка «ℹ️ Диагностика».
"""
...
# --- Gemini/Vertex (диагностика только; в этом файле не используется для генерации) ---
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")  # для Generative Language API
GOOGLE_APPLICATION_CREDENTIALS = os.getenv("GOOGLE_APPLICATION_CREDENTIALS")  # для Vertex AI (JSON credentials)
...
def _gemini_diag_lines() -> List[str]:
    lines = []
    # Gemini API key
    gk = GEMINI_API_KEY or ""
    lines.append(f"Gemini API key: {_ok(bool(gk))} {_mask(gk)}")
    # Vertex creds file
    cred = GOOGLE_APPLICATION_CREDENTIALS or ""
    exists = bool(cred and os.path.isfile(cred))
    lines.append(f"Vertex creds (JSON): {_ok(exists)} {cred or '—'}")
    # Дополнительно — переменные TG/Twitter/GitHub
    lines.append(f"TG approval bot: {_ok(bool(TELEGRAM_BOT_TOKEN_APPROVAL))}")
    lines.append(f"TG channel bot: {_ok(bool(TELEGRAM_BOT_TOKEN_CHANNEL))}")
    lines.append(f"TG channel id/username: {_ok(bool(TELEGRAM_CHANNEL_USERNAME_ID))} {TELEGRAM_CHANNEL_USERNAME_ID or '—'}")
    lines.append(f"Twitter v2 client: {_ok(bool(twitter_client_v2))}")
    lines.append(f"Twitter v1.1 API: {_ok(bool(twitter_api_v1))}")
    lines.append(f"GitHub repo: {_ok(bool(github_repo))} {GITHUB_REPO or '—'}")
    # Воркеры
    lines.append(f"Worker URL: {_ok(bool(AICOIN_WORKER_URL))} {AICOIN_WORKER_URL or '—'}")
    return lines

def build_diag_text() -> str:
    parts = [
        "<b>ℹ️ Диагностика окружения</b>",
        *(_gemini_diag_lines())
    ]
    return "\n".join(parts)
...
def build_tweet_with_tail_275(body_text: str, ai_tags: List[str] | None) -> str:
    ...
    if was_trimmed_initial and tail:
        allowed2 = MAX_TWEET - twitter_len(tail) - sep - 2
...
def get_start_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("ℹ️ Диагностика", callback_data="show_diag")],
    ...
    if data == "show_diag":
        try:
            ...
            await safe_send_message(
                approval_bot,
                chat_id=TELEGRAM_APPROVAL_CHAT_ID,
                text=f"⚠️ Не удалось показать диагностику: {e}",
                reply_markup=get_start_menu()
            )
        return
...
async def on_start(app: Application):
    ...
    try:
        await safe_send_message(approval_bot, chat_id=TELEGRAM_APPROVAL_CHAT_ID, text=build_diag_text(), parse_mode="HTML")
    except Exception as e:
        log.warning(f"Не удалось отправить диагностику на старте: {e}")
